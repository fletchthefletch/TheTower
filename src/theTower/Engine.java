...
public abstract class Engine implements KeyListener, MouseListener, MouseMotionListener {
		...
		public static void sleep(double ms) { // remove static
			try {
				// Sleep
		...
		public void setupWindow(int width, int height) {
			...
			mFrame.setTitle("The Tower");
			mFrame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE); //EXIT
					...
					.addKeyEventDispatcher(new KeyEventDispatcher() {
						...
						public boolean dispatchKeyEvent(KeyEvent e) {
							...
							case KeyEvent.KEY_PRESSED:
								Engine.this.keyPressed(e);
								return false;
							case KeyEvent.KEY_RELEASED:
								Engine.this.keyReleased(e);
								return false;
		...
		public static void createGame(Engine game, int framerate) {
			// Initialise Game
			game.init();
		...
		public static void createGame(Engine game) {
			// Call CreateGame
			createGame(game, fps);
		}
		...
		GameTimer timer = new GameTimer(fps, new ActionListener() {
			@Override
		...
		public void drawAlphaBoldText(double x, double y, String s, String font, int size, float alpha) {
			// Draw text on the screen
			mGraphics.setFont(new Font(font, Font.BOLD, size));
			mGraphics.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, alpha));
			mGraphics.drawString(s, (int)x, (int)y);
			mGraphics.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1));
		}
		...
		public void drawAlphaImage(Image image, double x, double y, double w, double h, float alpha)
		{
			// Check if image is null
			if(image == null) {
				// Print Error message
